...
$proyecto_sub = $_REQUEST['proyecto_sub'];

$objRprtePlnDsrrllo->setCodigoPlanDesarrollo($codigo_plandesarrollo);
$Subsistemas=$objRprtePlnDsrrllo->subSstmasPlanDsrrllo($sub_sistema);
$nombreNivelDos=$objRprtePlnDsrrllo->nivelDosNombre();
$nombreNivelTres=$objRprtePlnDsrrllo->nivelTresNombre();

$nombre_documento = $objRprtePlnDsrrllo->nombre_documento($codigo_plandesarrollo); 

$sigla_nivel_dos = $objRprtePlnDsrrllo->sigla_nivel_dos($proyecto_sub);

$nmbre_documento = $nombre_documento."_".$nombreNivelDos."_".$sigla_nivel_dos;
...
if($Subsistemas){//If Subsistema
    foreach ($Subsistemas as $data_subsistema) {//Forech Subsistema
        ...
        $proyectosSubsistema=$objRprtePlnDsrrllo->pryctosSbsstema($sub_codigo, $proyecto_sub);
            ...
            foreach ($proyectosSubsistema as $data_proyectosSubsistema) {//Foreach Proyectos
                ...
                if($accionProyecto){
                    foreach ($accionProyecto as $data_accionProyecto) {
                        ...
                        $descripcion_accion=$codigo_accion.'. '.strtolower(tildes($acc_descripcion));
                        ...
                        $objPHPExcel->setActiveSheetIndex($numero_registro)
                        ->setCellValue('E'.$num, strtolower(tildes($ind_unidadmedida)));
                        ...
                        for($inicioIndicadorVigencia=$yearinicio; $inicioIndicadorVigencia<=$yearfin; $inicioIndicadorVigencia++){
                            ...
                            $objPHPExcel->setActiveSheetIndex($numero_registro)
                            ->setCellValue(chr($celdaF+1).$num, $presupuesto);
                            $objPHPExcel->getActiveSheet($numero_registro)->getStyle(chr($celdaF+1).$num)->applyFromArray($texto_moneda);
                            $objPHPExcel->getActiveSheet($numero_registro)->getStyle(chr($celdaF+1).$num)->getNumberFormat()->setFormatCode('_("$"* #,##0_);_("$"* \(#,##0\);_("$"* "-"??_);_(@_)');                         
        ...
        $objPHPExcel->getActiveSheet($numero_excel)->getColumnDimension('A')->setWidth(26);
        ...
        $objPHPExcel->getActiveSheet($numero_excel)->getColumnDimension('D')->setWidth(13);
        $objPHPExcel->getActiveSheet($numero_excel)->getColumnDimension('E')->setWidth(20);

        //TAMAÑOS CELDAS
        $celdaF=70;
        for($inicioIndicadorVigencia=$yearinicio; $inicioIndicadorVigencia<=$yearfin; $inicioIndicadorVigencia++){
            $objPHPExcel->getActiveSheet($numero_excel)->getColumnDimension(chr($celdaF))->setWidth(11);
            $objPHPExcel->getActiveSheet($numero_excel)->getColumnDimension(chr($celdaF+1))->setWidth(18);
...
header('Content-Disposition: attachment;filename="'.$nmbre_documento.'.xlsx"');
