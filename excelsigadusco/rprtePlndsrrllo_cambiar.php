...
->setCategory("Plan Desarrollo");
  ...
  if($Subsistemas){//If Subsistema
    foreach ($Subsistemas as $data_subsistema) {//Forech Subsistema
      ...
      if($proyectosSubsistema){// if Proyecto
        foreach ($proyectosSubsistema as $data_proyectosSubsistema) {//Foreach Proyectos
          ...
          for($inicioIndicadorVigencia=$yearinicio; $inicioIndicadorVigencia<=$yearfin; $inicioIndicadorVigencia++){
            ...
            $sheet->mergeCells(chr($celdaE).($num).":".chr($celdaE+1).($num));
            $objPHPExcel->setActiveSheetIndex($numero_registro)
            ->setCellValue(chr($celdaE).$num, $inicioIndicadorVigencia);
            $objPHPExcel->getActiveSheet($numero_registro)->getStyle(chr($celdaE).($num).":".chr($celdaE+1).($num))->applyFromArray($titulo_center);
          ...
          for($inicioIndicadorVigencia=$yearinicio; $inicioIndicadorVigencia<=$yearfin; $inicioIndicadorVigencia++){
            ...
            $objPHPExcel->setActiveSheetIndex($numero_registro)
            ->setCellValue(chr($celdaE).$num, 'UNIDAD');
            $objPHPExcel->getActiveSheet($numero_registro)->getStyle(chr($celdaE).$num)->applyFromArray($titulo_center);

            $objPHPExcel->setActiveSheetIndex($numero_registro)
            ->setCellValue(chr($celdaE+1).$num, 'PRESUPUESTO');
            $objPHPExcel->getActiveSheet($numero_registro)->getStyle(chr($celdaE+1).$num)->applyFromArray($titulo_center);
          ...
          if($accionProyecto){
            foreach ($accionProyecto as $data_accionProyecto) {
              ...
              $inidicadorAccion=$objRprtePlnDsrrllo->indicadorAccion($acc_codigo);
                if($inidicadorAccion){
                  foreach ($inidicadorAccion as $data_indicadorAccion) {
                    $ind_codigo=$data_indicadorAccion['ind_codigo'];
                    $ind_unidadmedida=$data_indicadorAccion['ind_unidadmedida'];
                    $ind_lineabase=$data_indicadorAccion['ind_lineabase'];
                    $ind_metaresultado=$data_indicadorAccion['ind_metaresultado'];
              ...
              for($inicioIndicadorVigencia=$yearinicio; $inicioIndicadorVigencia<=$yearfin; $inicioIndicadorVigencia++){
                ...
                $objPHPExcel->setActiveSheetIndex($numero_registro)
                ->setCellValue(chr($celdaE).$num, $unidad);
                $objPHPExcel->getActiveSheet($numero_registro)->getStyle(chr($celdaE).$num)->applyFromArray($texto_center);
    
                $objPHPExcel->setActiveSheetIndex($numero_registro)
                ->setCellValue(chr($celdaE+1).$num, $presupuesto);
                $objPHPExcel->getActiveSheet($numero_registro)->getStyle(chr($celdaE+1).$num)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_CURRENCY_USD);
                $objPHPExcel->getActiveSheet($numero_registro)->getStyle(chr($celdaE+1).$num)->applyFromArray($texto_center);
            ...
            for($inicioIndicadorVigencia=$yearinicio; $inicioIndicadorVigencia<=$yearfin; $inicioIndicadorVigencia++){
              ...
              $objPHPExcel->setActiveSheetIndex($numero_registro)
              ->setCellValue(chr($celdaE).$num, '');
              $objPHPExcel->getActiveSheet($numero_registro)->getStyle(chr($celdaE).$num)->applyFromArray($titulo_center);

              $objPHPExcel->setActiveSheetIndex($numero_registro)
              ->setCellValue(chr($celdaE+1).$num, '');
              $objPHPExcel->getActiveSheet($numero_registro)->getStyle(chr($celdaE+1).$num)->applyFromArray($titulo_center);
        ...
        for($inicioIndicadorVigencia=$yearinicio; $inicioIndicadorVigencia<=$yearfin; $inicioIndicadorVigencia++){
          ...
          $objPHPExcel->setActiveSheetIndex($numero_registro)
          ->setCellValue(chr($celdaE+1).$num, $valorProyectoVigencia);
          $objPHPExcel->getActiveSheet($numero_registro)->getStyle(chr($celdaE+1).$num)->applyFromArray($texto_center);
          $objPHPExcel->getActiveSheet($numero_registro)->getStyle(chr($celdaE+1).$num)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_CURRENCY_USD);
      ...
      $objPHPExcel->getActiveSheet($numero_excel)->getColumnDimension('E')->setWidth(12);
      $objPHPExcel->getActiveSheet($numero_excel)->getColumnDimension('F')->setWidth(20);
      $objPHPExcel->getActiveSheet($numero_excel)->getColumnDimension('G')->setWidth(12);
