...
$rfrncia_accion = $objRprteRP->rfrncia_accion($codigo_accion);

$nombre_archvo = $nombre_nivel_tres."_".$rfrncia_accion."_".$fecha_generar;
...
$num = 1;
$list_proyecto = $objRprteRP->list_proyecto($codigo_proyecto);
if($list_proyecto){
    foreach ($list_proyecto as $dat_lsta_prycto) {
        $pro_codigo = $dat_lsta_prycto['pro_codigo'];
        $pro_referencia = $dat_lsta_prycto['pro_referencia']; 
        $pro_numero = $dat_lsta_prycto['pro_numero'];
        $pro_descripcion = $dat_lsta_prycto['pro_descripcion'];
        $pro_objetivo = $dat_lsta_prycto['pro_objetivo'];
        ...
        $list_acciones_codigo = $objRprteRP->list_acciones_codigo($pro_codigo, $codigo_accion);
        if($list_acciones_codigo){
            foreach($list_acciones_codigo as $dat_acciones){
                                ...
                                if($list_cdp){
                                    foreach ($list_cdp as $dat_cdp) {
                                        ...
                                        if($datos_rp){
                                            ...
                                            //$objPHPExcel->getActiveSheet()->getStyleByColumnAndRow("R", $num)->getNumberFormat()->setFormatCode('[$-C09]d mmm yyyy;@');
...
if($list_proyecto){
    foreach ($list_proyecto as $dat_lsta_prycto) {
        ...
        $list_accion_ejecucion = $objRprteRP->list_accion_ejecucion($pro_codigo, $codigo_accion, $codigo_vigencia);
        if($list_accion_ejecucion){
            foreach ($list_accion_ejecucion as $dta_accines_ejccion) {
                $acc_codigo = $dta_accines_ejccion['acc_codigo'];
                $acc_referencia = $dta_accines_ejccion['acc_referencia']; 
                $acc_numero = $dta_accines_ejccion['acc_numero'];
                $acc_descripcion = $dta_accines_ejccion['acc_descripcion']; 
                $acc_proyecto = $dta_accines_ejccion['acc_proyecto'];
                $ind_codigo = $dta_accines_ejccion['ind_codigo'];
                $ind_unidadmedida = $dta_accines_ejccion['ind_unidadmedida']; 
                $ind_lineabase = $dta_accines_ejccion['ind_lineabase'];
                $ind_metaresultado = $dta_accines_ejccion['ind_metaresultado'];
                $ind_sede = $dta_accines_ejccion['ind_sede'];
                $ivi_presupuesto = $dta_accines_ejccion['ivi_presupuesto'];
                $ind_tipocomportamiento = $dta_accines_ejccion['ind_tipocomportamiento'];
