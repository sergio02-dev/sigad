...
$list_fuente_financiacion = $objReportePoai->list_fuente_financiacion($codigo_plan, $vigencia);
...
if($lista_poai){
    foreach ($lista_poai as $dta_list_poai) {
        ...
        if($list_fuente_financiacion){
            foreach ($list_fuente_financiacion as $dta_lsta_fuente_financiacion) {
                ...
                $recurso_poai = $objReportePoai->valor_poai($vigencia_fuente, $acc_codigo, $ffi_codigo, $ind_codigo, $vigencia);

                $total_poai_indicador = $total_poai_indicador + $recurso_poai;

                $objPHPExcel->setActiveSheetIndex($numero_registro)
                ->setCellValue($letra.$num_registro, $recurso_poai);
    ...
    if($list_fuente_financiacion){
        ...
        foreach ($list_fuente_financiacion as $dta_lsta_fuente_financiacion) {
            ...
            //$totalizado_fuente_poai = $objReportePoai->totalizado_fuente_poai($vigencia, $ffi_codigo);
            $totalizado_fuente_poai = $objReportePoai->sum_fuente_plan($codigo_plan, $ffi_codigo, $vigencia_fuente, $vigencia);
        ...
        foreach ($list_subsistema as $data_subsistema) {
            ...
            if($list_fuente_financiacion){
                ...
                foreach ($list_fuente_financiacion as $dta_lsta_fuente_financiacion) {
                    ...
                    $suma_poai_subsistema = $objReportePoai->suma_poai_subsistema($codigo_plan, $sub_codigo, $vigencia, $ffi_codigo, $vigencia_fuente);
    ...
    $list_trsldos_plan = $objReportePoai->lista_traslados_plan($codigo_plan);
...
$objPHPExcel->getActiveSheet($numero_excel)->getColumnDimension('A')->setWidth(38);
