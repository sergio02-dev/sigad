<?php 
Class RsRprteAsgncionRcrsoPlanAccion{
    ...
    public function fuentes_asignadas_accion($codigo_accion){
        
        $sql_fuentes_asignadas_accion="SELECT DISTINCT asre_fuente, asre_vigenciarecurso, ffi_nombre
                                         FROM planaccion.poai
                                        INNER JOIN planaccion.asignacion_recuersos_etapa ON poa_codigo = asre_etapa
                                        INNER JOIN planaccion.fuente_financiacion ON asre_fuente = ffi_codigo
                                        WHERE acp_codigo = $codigo_accion
                                        ORDER BY asre_vigenciarecurso ASC";

        $resultado_fuentes_asignadas_accion=$this->cnxion->ejecutar($sql_fuentes_asignadas_accion);

        while ($data_fuentes_asignadas_accion = $this->cnxion->obtener_filas($resultado_fuentes_asignadas_accion)){
            $datafuentes_asignadas_accion[] = $data_fuentes_asignadas_accion;
        }
        return $datafuentes_asignadas_accion;
    }

    public function recurso_asignado($codigo_etapa, $codigo_fuente, $vigencia_recurso){
        ...
        $asre_recurso = $data_recurso_asignado['asre_recurso'];

        if($asre_recurso){
            $total_recurso = $asre_recurso;
        }
        else{
            $total_recurso = 0; 
        }
