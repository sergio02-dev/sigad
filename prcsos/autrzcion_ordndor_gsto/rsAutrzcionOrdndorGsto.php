 ...
 * 04 de Mayo 2023 12:47pm
 * Rs Autorizacion Ordenador del Gasto
 */
include('classAutrzcionOrdndorGsto.php');
class RsAutrzcionOrdndorGsto extends AutorizacionOrdenadorGasto{
    private $cnxion;
    ...
    public function autorizacion_financiera($codigo_solicitud){

        $sql_autorizacion_financiera="SELECT COUNT(*) AS aprovacion_financiera
                                        FROM cdp.aprovacion_solicitud
                                       WHERE asol_clasificacion = 2
                                         AND asol_solicitud = $codigo_solicitud;";

        $query_autorizacion_financiera=$this->cnxion->ejecutar($sql_autorizacion_financiera);

        $data_autorizacion_financiera=$this->cnxion->obtener_filas($query_autorizacion_financiera);

        $aprovacion_financiera = $data_autorizacion_financiera['aprovacion_financiera'];

        return $aprovacion_financiera;
    }
    ...
    public function autorizacion_ordenador_gasto($codigo_solicitud){

        $sql_autorizacion_ordenador_gasto="SELECT COUNT(*) AS aprovacion_ordenador
                                             FROM cdp.aprovacion_solicitud
                                            WHERE asol_clasificacion = 4
                                              AND asol_solicitud = $codigo_solicitud;";

        $query_autorizacion_ordenador_gasto=$this->cnxion->ejecutar($sql_autorizacion_ordenador_gasto);

        $data_autorizacion_ordenador_gasto=$this->cnxion->obtener_filas($query_autorizacion_ordenador_gasto);

        $aprovacion_ordenador = $data_autorizacion_ordenador_gasto['aprovacion_ordenador'];

        return $aprovacion_ordenador;
    }
    ...
    public function actividades_solicitud($codigo_solicitud){
        ...
        $sql_actividades_solicitud = "SELECT DISTINCT aes_actividad, acp_referencia, 
                                       ...
                                       WHERE aes_solicitud = $codigo_solicitud
                                         AND aes_actividad IN(SELECT DISTINCT ascl_actividad 
                                                                FROM cdp.aprovacion_solicitud
                                                               INNER JOIN cdp.aprovacion_solicitud_clasificador ON asol_codigo = ascl_aprovacionsolicitud
                                                               WHERE asol_clasificacion = 2
                                                                 AND ascl_aprovacion = 1
    ...
    public function etapa_solicitud($codigo_solicitud, $codigo_actividad){
        ...
        $sql_etapa_solicitud = "SELECT aes_codigo, aes_solicitud, aes_actividad, 
                                 ...
                                 WHERE aes_solicitud = $codigo_solicitud
                                   ...
                                   AND aes_etapa IN(SELECT DISTINCT ascl_etapa 
                                                      FROM cdp.aprovacion_solicitud
                                                     INNER JOIN cdp.aprovacion_solicitud_clasificador ON asol_codigo = ascl_aprovacionsolicitud
                                                     WHERE asol_clasificacion = 2
                                                       AND ascl_aprovacion = 1
    ...
    public function clasificador_etapa($codigo_solicitud, $codigo_etapa){
